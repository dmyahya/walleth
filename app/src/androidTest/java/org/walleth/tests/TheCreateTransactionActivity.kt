...
class TheCreateTransactionActivity {
    ...
    fun rejectsDifferentChainId() {
        ...
        Espresso.onView(ViewMatchers.withText(rule.activity.getString(R.string.alert_network_unsupported_message, TestApp.networkDefinitionProvider.getCurrent().getNetworkName(), chainIdForTransaction)))
                .check(ViewAssertions.matches(ViewMatchers.isDisplayed()))
